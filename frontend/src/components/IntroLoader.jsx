...
export default function IntroLoader({ onComplete }) {
  ...
  useEffect(() => {
    ...
    const build = () => {
      ...
      const area = w * h;
      const n = Math.min(190, Math.max(68, Math.floor(area / 9500)));
      const list = [];
      for (let i = 0; i < n; i += 1) {
        list.push({
          x: Math.random() * w,
    ...
    const paint = (t) => {
      ...
      for (let i = 0; i < list.length; i += 1) {
        ...
        const regionBoost = 0.46 + 0.52 * Math.min(1, nx / Math.max(0.08, pLoad + 0.04));
        const distWave = Math.abs(p.x - edgeX);
        const waveBoost = 1 + Math.max(0, 1 - distWave / 120) * 0.38 * Math.sin(elapsed * 3.6 + p.phase);
        const pulse = 0.58 + 0.42 * Math.sin(elapsed * 1.75 + p.phase);
        ...
        if (p.kind === 'gold') {
          ctx.fillStyle = `rgba(184, 154, 104, ${a * 0.78})`;
        } else {
          ctx.fillStyle = `rgba(26, 31, 36, ${a * 0.42})`;
        }
        ctx.fill();
      }
  ...
  useEffect(() => {
    ...
    const minHoldMs = reduced ? 400 : 3200;
    const rampMs = reduced ? 1 : 2800;
    const exitMs = reduced ? 300 : 680;
  ...
  return (
    ...
    >
      ...
      <div className={styles.loaderShell} style={{ '--pct': `${pct}%` }}>
        ...
        <div className={styles.loaderMeter} aria-hidden>
          <span />
          <b />
        </div>
