...
function CursorParticles() {
  return (
    <div className="dp2-cursor-particles" aria-hidden>
      {cursorTrail.map(([x, y, opacity], index) => (
        <i
          key={index}
          style={{
            '--i': index,
    ...
    <PageShell variant="loading">
      <div className="dp2-loader">
        ...
        <div className="dp2-loader-bottom">
          <span>PROFILE / CHAPTER / SESSION</span>
          <strong>72%</strong>
          <span>READY</span>
        </div>
      </div>
    ...
    <PageShell variant="quiz">
      <div className="dp2-quiz">
        <section className="dp2-section-title">
          ...
          <h2>章节小测</h2>
          <p>15 题同屏滚动，选项变成更清楚的大按钮；可直接进入测试，也可在章节结束后自动出现。</p>
        </section>
    ...
    <PageShell variant="result">
      <div className="dp2-result">
        <section className="dp2-result-hero">
          ...
          <h2>结果解析</h2>
          <p>不跳页，不弹窗。当前窗口直接展示掌握判断、答案解析和下一轮 AI 巩固路径。</p>
        ...
        <section className="dp2-result-board">
          <div className="dp2-result-status">
            <span>STATUS</span>
            <strong>未达标，继续学习</strong>
            <p>系统会把错题对应到知识网络，并把下一轮对话聚焦到薄弱节点。</p>
          </div>
